...
?>"
    <?php 
echo file_get_contents(__DIR__ . '/toolbar.js');
?>
    <?php 
echo file_get_contents(__DIR__ . '/toolbarstandalone.js');
?>
</script>
...
foreach ($collectors as $c) {
    ?>
            <?php 
    if (!$c['isEmpty'] && ($c['hasTabContent'] || $c['hasLabel'])) {
        ?>
                <span class="ci-label">
                    <a data-tab="ci-<?php 
        echo $c['titleSafe'];
...
?>
            ...
            <tbody>
            <?php 
echo $this->render_timeline($collectors, $start_time, $segment_count, $segment_duration, $styles);
?>
            </tbody>
        </table>
    </div>
    ...
    if (!$c['isEmpty']) {
        ...
        if ($c['hasTabContent']) {
            ...
            ?></span></h2>
                    ...
                    <?php 
            echo is_string($c['display']) ? $c['display'] : $parser->set_data($c['display'])->render("_{$c['titleSafe']}.tpl");
            ?>
                </div>
            <?php 
        }
...
?>
    ...
    <div id="ci-vars" class="tab">
        ...
        <?php 
if (isset($vars['varData'])) {
    ?>
            <?php 
    foreach ($vars['varData'] as $heading => $items) {
        ?>

                <a class="debug-bar-vars" data-toggle="datatable" data-table="<?php 
        echo strtolower(str_replace(' ', '-', $heading));
        ?>">
                    <h2><?php 
        echo $heading;
        ?></h2>
        ...
        if (is_array($items)) {
            ?>
                    ...
                    <table id="<?php 
            echo strtolower(str_replace(' ', '-', $heading . '_table'));
            ?>">
                        <tbody>
                        <?php 
...
?> )</span></h2>
        ...
        <?php 
if (isset($vars['get']) && $get = $vars['get']) {
    ?>
...
?>
        ...
        <?php 
if (isset($vars['post']) && $post = $vars['post']) {
    ?>
...
?>
        ...
        <?php 
if (isset($vars['headers']) && $headers = $vars['headers']) {
    ?>
...
?>
        ...
        <?php 
if (isset($vars['cookies']) && $cookies = $vars['cookies']) {
    ?>
...
?>
        ...
        <h2>Response
            <span>( <?php 
echo $vars['response']['statusCode'] . ' - ' . $vars['response']['reason'];
?> )</span>
        </h2>

        <?php 
if (isset($vars['response']['headers']) && $headers = $vars['response']['headers']) {
    ?>
...
?>
    ...
    <div id="ci-config" class="tab">
        ...
        <?php 
echo $parser->set_data($config)->render('_config.tpl');
?>
    </div>
</div>
<style>
