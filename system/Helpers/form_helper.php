    ...
    function form_open(string $action = '', $attributes = [], array $hidden = []): string
    {
        ...
        if ($action === '') {
            ...
            if (str_contains($action, '{locale}')) {
                $action = str_replace('{locale}', service('request')->get_locale(), $action);
            }
        ...
        $before = service('filters')->get_filters()['before'];
...
if (!function_exists('form_dropdown')) {
    ...
    function form_dropdown($data = '', $options = [], $selected = [], $extra = ''): string
    {
        ...
        if ($selected === []) {
            ...
            if (is_array($data)) {
                $post_value = $superglobals->post($data['name'] ?? '');
                if (isset($data['name']) && $post_value !== null) {
                    $selected = [$post_value];
                }
            } else {
                $post_value = $superglobals->post($data);
                if ($post_value !== null) {
                    $selected = [$post_value];
                }
            }
        ...
        foreach ($options as $key => $val) {
            ...
            if (is_array($val)) {
                ...
                foreach ($val as $optgroup_key => $optgroup_val) {
                    ...
                    $form .= '<option value="' . htmlspecialchars($optgroup_key) . '"' . $sel . '>' . $optgroup_val . "</option>\n";
                }
                $form .= "</optgroup>\n";
            } else {
...
if (!function_exists('set_value')) {
     ...
     */
    function set_value(string $field, $default = '', bool $html_escape = true)
    {
        $request = service('request');
        // Try any old input data we may have first
        $value = $request->get_old_input($field);
        if ($value === null) {
            $value = $request->get_post($field) ?? $default;
        }
...
if (!function_exists('set_select')) {
    ...
    function set_select(string $field, string $value = '', bool $default = false): string
    {
        $request = service('request');
        // Try any old input data we may have first
        $input = $request->get_old_input($field);
        if ($input === null) {
            $input = $request->get_post($field);
        }
        if ($input === null) {
...
if (!function_exists('set_checkbox')) {
    ...
    function set_checkbox(string $field, string $value = '', bool $default = false): string
    {
        ...
        $has_old_input = $session->has('_ci_old_input');
        // Unchecked checkbox and radio inputs are not even submitted by browsers ...
        if ((string) $input === '0' || !empty($request->get_post()) || $has_old_input) {
...
if (!function_exists('set_radio')) {
    ...
    function set_radio(string $field, string $value = '', bool $default = false): string
    {
        $request = service('request');
        // Try any old input data we may have first
        $old_input = $request->get_old_input($field);
        $post_input = $request->get_post($field);
        $input = $old_input ?? $post_input ?? $default;
        if (is_array($input)) {
            // Note: in_array('', array(0)) returns TRUE, do not use it
            foreach ($input as $v) {
                if ($value === $v) {
        ...
        // Unchecked checkbox and radio inputs are not even submitted by browsers ...
        if ($old_input !== null || $post_input !== null) {
...
if (!function_exists('validation_list_errors')) {
    ...
    function validation_list_errors(string $template = 'list'): string
    {
        ...
        if (!array_key_exists($template, $config->templates)) {
            throw Validation_Exception::for_invalid_template($template);
        }
        return $view->set_var('errors', validation_errors())->render($config->templates[$template]);
    }
}
if (!function_exists('validation_show_error')) {
    ...
    function validation_show_error(string $field, string $template = 'single'): string
    {
        $config = config(Validation::class);
        $view = service('renderer');
        $errors = array_filter(validation_errors(), static fn($key): bool => preg_match('/^' . str_replace(['\.\*', '\*\.'], ['\..+', '.+\.'], preg_quote($field, '/')) . '$/', $key) === 1, ARRAY_FILTER_USE_KEY);
        if ($errors === []) {
            return '';
        }
        if (!array_key_exists($template, $config->templates)) {
        ...
        return $view->set_var('error', implode("\n", $errors))->render($config->templates[$template]);
    }
}
