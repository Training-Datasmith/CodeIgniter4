...
final class Factories
{
     ...
     */
    private static array $config_options = ['component' => 'config', 'path' => 'Config', 'instanceOf' => null, 'getShared' => true, 'preferApp' => true];
    /**
    ...
    public static function define(string $component, string $alias, string $classname): void
    {
        ...
        if (isset(self::$aliases[$component][$alias])) {
            ...
            throw new InvalidArgumentException('Already defined in Factories: ' . $component . ' ' . $alias . ' -> ' . self::$aliases[$component][$alias]);
        }
    ...
    public static function __callStatic(string $component, array $arguments)
    {
        ...
        $options = array_merge(self::get_options($component), $options);
        if (!$options['getShared']) {
            ...
            $class = self::locate_class($options, $alias);
            if ($class !== null) {
                return new $class(...$arguments);
            }
            return null;
        ...
        $instance = self::get_defined_instance($options, $alias, $arguments);
        if ($instance !== null) {
            return $instance;
        }
        // Try to locate the class
        if (($class = self::locate_class($options, $alias)) === null) {
            return null;
        }
        self::create_instance($options['component'], $class, $arguments);
        self::set_alias($options['component'], $alias, $class);
     ...
     * @return object|null
     */
    private static function get_defined_instance(array $options, string $alias, array $arguments)
    {
     ...
     */
    private static function create_instance(string $component, string $class, array $arguments): void
    {
     ...
     * Sets alias
     */
    private static function set_alias(string $component, string $alias, string $class): void
    {
        self::$aliases[$component][$alias] = $class;
        self::$updated[$component] = true;
        // If a short classname is specified, also register FQCN to share the instance.
        if (!isset(self::$aliases[$component][$class]) && !self::is_namespaced($alias)) {
     ...
     * @param string $component Lowercase, plural component name
     */
    private static function is_config(string $component): bool
    {
     ...
     * @param string $alias   Class alias. See the $aliases property.
     */
    private static function locate_class(array $options, string $alias): ?string
    {
        // Check for low-hanging fruit
        if (class_exists($alias, false) && self::verify_prefer_app($options, $alias) && self::verify_instance_of($options, $alias)) {
            return $alias;
        }
        // Determine the relative class names we need
        $basename = self::get_basename($alias);
        $appname = self::is_config($options['component']) ? 'Config\\' . $basename : rtrim(APP_NAMESPACE, '\\') . '\\' . $options['path'] . '\\' . $basename;
        // If an App version was requested then see if it verifies
        if (!self::is_namespaced($alias) && $options['preferApp'] && class_exists($appname) && self::verify_instance_of($options, $alias)) {
            return $appname;
        }
        // If we have ruled out an App version and the class exists then try it
        if (class_exists($alias) && self::verify_instance_of($options, $alias)) {
            return $alias;
        }
        ...
        if (self::is_namespaced($alias)) {
            if (!$file = $locator->locate_file($alias, $options['path'])) {
                return null;
            }
            $files = [$file];
        ...
        foreach ($files as $file) {
            $class = $locator->find_qualified_name_from_path($file);
            if ($class !== false && self::verify_instance_of($options, $class)) {
                return $class;
            }
        }
        return null;
     ...
     * @param string $alias Class alias. See the $aliases property.
     */
    private static function is_namespaced(string $alias): bool
    {
     ...
     * @param string $alias   Class alias. See the $aliases property.
     */
    private static function verify_prefer_app(array $options, string $alias): bool
    {
        ...
        if (self::is_config($options['component'])) {
     ...
     * @param string $alias   Class alias. See the $aliases property.
     */
    private static function verify_instance_of(array $options, string $alias): bool
    {
        // Anything without that restriction passes
    ...
    public static function get_options(string $component): array
    {
        ...
        $values = self::is_config($component) ? self::$config_options : config('Factory')->{$component} ?? [];
    ...
    {
        if ($component !== null) {
            unset(self::$options[$component], self::$aliases[$component], self::$instances[$component], self::$updated[$component]);
            return;
        }
        self::$options = [];
        self::$aliases = [];
     ...
     * @testTag
     */
    public static function inject_mock(string $component, string $alias, object $instance)
    {
        $component = strtolower($component);
        // Force a configuration to exist for this component
        self::get_options($component);
        ...
        self::$aliases[$component][$alias] = $class;
        if (self::is_config($component)) {
            if (self::is_namespaced($alias)) {
                self::$aliases[$component][self::get_basename($alias)] = $class;
            } else {
     ...
     * @testTag
     */
    public static function get_basename(string $alias): string
    {
     ...
     * @internal For caching only
     */
    public static function get_component_instances(string $component): array
    {
        if (!isset(self::$aliases[$component])) {
            return ['options' => [], 'aliases' => [], 'instances' => []];
        }
        return ['options' => self::$options[$component], 'aliases' => self::$aliases[$component], 'instances' => self::$instances[$component]];
    }
    /**
     * Sets component data
     *
     * @internal For caching only
     */
    public static function set_component_instances(string $component, array $data): void
    {
     ...
     * @internal For caching only
     */
    public static function is_updated(string $component): bool
    {
