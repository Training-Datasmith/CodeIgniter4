...
class Escaper implements Escaper_Interface
{
     ...
     * @var int
     */
    protected $html_special_chars_flags;
    /**
    ...
    public function __construct(?string $encoding = null)
    {
        ...
        $this->html_attr_matcher = fn(array $matches): string => $this->html_attr_matcher($matches);
        $this->js_matcher = fn(array $matches): string => $this->js_matcher($matches);
        $this->css_matcher = fn(array $matches): string => $this->css_matcher($matches);
    }
    /**
    ...
    public function escape_html_attr(string $string)
    {
        ...
        $result = preg_replace_callback('/[^a-z0-9,\.\-_]/iSu', $this->html_attr_matcher, $string);
    ...
    public function escape_js(string $string)
    {
        ...
        $result = preg_replace_callback('/[^a-z0-9,\._]/iSu', $this->js_matcher, $string);
        assert(is_string($result));
        return $this->from_utf8($result);
    }
    /** @inheritDoc */
    ...
    public function escape_css(string $string)
    {
        ...
        $result = preg_replace_callback('/[^a-z0-9]/iSu', $this->css_matcher, $string);
        assert(is_string($result));
        return $this->from_utf8($result);
    }
    /**
     ...
     * @return string
     */
    protected function html_attr_matcher($matches)
    {
        $chr = $matches[0];
        $ord = ord($chr[0]);
        /**
     ...
     * @return string
     */
    protected function js_matcher($matches)
    {
        $chr = $matches[0];
        if (strlen($chr) === 1) {
     ...
     * @return string
     */
    protected function css_matcher($matches)
    {
        $chr = $matches[0];
        if (strlen($chr) === 1) {
            $ord = ord($chr);
     ...
     * @return string
     */
    protected function convert_encoding($string, $to, $from)
    {
